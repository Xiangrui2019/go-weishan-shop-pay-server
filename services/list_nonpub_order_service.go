...
func (service *ListNonPubOrderService) List() *serializer.Response {
	...
	if err := models.DB.Model(models.Order{}).Where("status = ?", true).Count(&total).Error; err != nil {
	...
	if err := models.DB.Where("status = ?", true).Limit(service.Limit).Offset(service.Start).Find(&orders).Error; err != nil {
		return &serializer.Response{
			Code:    http.StatusInternalServerError,
			Message: "数据库连接错误",
			Error:   err.Error(),
